...
3. Adjust the fan speed with the knob on the RPM Controller while reading the RPM from the Fan Speed Display.  Adjust the fan speed from 15% to 85% of max fan speed (in increments of 10%). At each fan speed, measure the airspeed.
...
Dimensions of a "flat plate wing" inspired by https://grabcad.com/library/naca0012-airfoil-1. Note that this is NOT a flat plate wing, and you should not recreate this exact geometry. Instead, you should create a flat plate wing with the same reference area as this geometry.
:::
