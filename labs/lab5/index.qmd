...
6. Capture an oscilloscope trace for each of the signals below. 
    1. Signal 1, which is found by following steps 1 - 5
    2. Signal 2 
    3. Signal 3 
    4. Signal 3 with the the "window" setting changed from rectangular to Hanning.
7. Capture some oscilloscope data and take an FFT of it using Matlab, then compare that FFT to the scope FFTs.  Do so by following these steps.
    ...
    2. Drive signal 3 into the oscilloscope
    3. Ensure the "max length" setting is selected on the "save" menu and save the waveform 
    4. Take the FFT of that data in Matlab with a Hanning window   

## Circuit Interfaces to Microphones {#sec-interface}

You will need to use microphones to pick up sounds in the tank. We will be using a waterproof electret microphone called the [CME-1538-100LB](https://www.sameskydevices.com/product/resource/cme-1538-100lb.pdf). 
