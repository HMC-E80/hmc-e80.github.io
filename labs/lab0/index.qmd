...
Submission Sheet: Submit your [E72+E80 MATLAB Tutorial](./matlab-startup.qmd) on the E80 Canvas page

Pre-lab Help Sheet: No pre-lab required this week
...
This lab is a little different than most E80 labs because we want each team member to individually practice most of the skills. 
In future labs,  it will be **imperative** to get into the habit of distributing work across team members and **working in parallel.** Deciding how to break up that work is a great pre-lab exercise, and you'll be hearing more about pre-lab soon.
...
4. Keep moving around the circle until the proverb feels complete.
...
Your AUV will need power, and we provide that with a battery pack. The battery pack used in E80 is a 3000 mAh to 3500 mAh, 40 C, 11.1 V LiPo battery pack. 
To charge the batteries, we will use a [HiTec x1 Mini charger](https://drive.google.com/file/d/1q7Cchnw4c7wzFiuOYf5PHiICkSoq8qX5/view?usp=drive_link) or a [UP60AC multi charger](https://drive.google.com/file/d/1T9ghDlmI2UUg3yBbOCQxVdA6DsFghrLr/view?usp=drive_link). Here are some helpful [images](https://drive.google.com/open?id=1K1DSOdeqAjU_JU-sTOdiBUnwMmpwUTPP) showing a correct charging setup. 
...
Test your connection to the Teensy by compiling and uploading the RGB LED example in the Arduino IDE.  Then ensure you can compile and upload the E80 code.
...
:::{.callout-note}
You will not have a Teensy for pre-labs, but downloading the necessary software and trying to modify and compile your lab code in the IDE is great pre-lab practice.
:::
...
In future labs, you will use MATLAB to read binary data saved on an SD card by the Teensy.
The data is stored in two separate files:

1. An `inf` file (`.txt` extension) which lists the variable names and types
2. A `log` file (`.bin` extension) which contains the raw binary data. 

::: {.callout-warning}
Both the `inf` and `log` files are needed to import your data.
:::

Here is an example of the two files that would be found on an SD card after a run:

- [inf004.txt](./assets/INF004.TXT)
- [log004.bin](./assets/LOG004.BIN)

Use the provided MATLAB script, [logreader.m]({{< var github-url >}}/blob/main/MATLAB/logreader.m) (found under the `MATLAB` directory of the E80 repo you should have cloned), to read the data from the sample files.

::: {.callout-note}
The best practice is to use the file that is in the Git repository that you cloned instead of using another downloaded copy.
This way, if we update the file in the future you can easily pull down the newest version and see the changes you've made compared to the original file.
:::


1. Open `logreader.m` with MATLAB
2. Move both files into your Current Folder (see @fig-current-folder for a screenshot).
3. Change the `filenum` variable to match the number of the files. For the sample files, this would be `004`.
4. Plot one of the variable to check that data was read correctly (e.g., `plot(accelX)`)

::: {#fig-current-folder}
![](img/matlab_lab0.png)

Screenshot showing how to place files in Matlab current folder.
:::
