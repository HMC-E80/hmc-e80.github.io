...
## Binary File Reading
In future labs, you will use MATLAB to read binary data saved on an SD card by the Teensy. The data is stored in two separate files: an `inf` file, which lists the variable names and types, and a `log` file, which contains the raw binary data. Both are needed to read the data.

Here is an example of the two files that would be found on an SD card after a run:

- [inf004.txt](https://drive.google.com/open?id=18-MICAN9QopCYHtpTnUVNme07YvaUmFD)\
- [log004.bin](https://drive.google.com/open?id=1h086niwvAh4C3Gs6r4vE12ZvaD1KYria)

Use the provided MATLAB script, [logreader.m](https://github.com/HMC-E80/E80-SP2023/blob/main/MATLAB/logreader.m) [found under the `MATLAB` directory of the E80 repo you should have cloned], to read the data from the sample files.


1. Open `logreader.m` with MATLAB
2. Move both files into your Current Folder
![](img/matlab_lab0.png){#fig-matlab-currdir}
